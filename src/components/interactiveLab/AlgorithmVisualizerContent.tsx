...
import { AlgorithmDetails } from "../interactive/AlgorithmDetails";
...
export function AlgorithmVisualizerContent() {
  ...
  const quickSort = useCallback(async (array: number[], startIndex = 0, endIndex = array.length - 1) => {
    if (cancelReference.current || startIndex >= endIndex) return;
    ...
    for (let currentIndex = startIndex; currentIndex < endIndex; currentIndex++) {
      if (cancelReference.current) return;
      setActiveIndices([currentIndex, endIndex]);
    ...
    updateBars(array);
    setSortedIndices((previous) => [...previous, pivotIndex]);
    await sleep(ANIMATION_DELAY_MS);

    await quickSort(array, startIndex, pivotIndex - 1);
    await quickSort(array, pivotIndex + 1, endIndex);
  }, [updateBars]);

  const merge = useCallback(async (array: number[], startIndex: number, middleIndex: number, endIndex: number) => {
  ...
  const mergeSort = useCallback(async (array: number[], startIndex = 0, endIndex = array.length - 1): Promise<void> => {
    if (cancelReference.current || startIndex >= endIndex) return;
